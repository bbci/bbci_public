...
%
% 2013: matthias: added logical conversion to "true" and "false" parameters
...
global pos inStr len
    % Search for a float/double item with a period "."
    [matchstart,matchend,~,item]=regexp(inStr(pos:end),'^\s*\d+\.\d*');
    err=[];
    if ~isempty(item)
        num= double(str2num(item{:}));
    else
        % Should be an integer
        [matchstart,matchend,~,item]=regexp(inStr(pos:end),'^\s*\d+');
        if ~isempty(item)
            num= int64(str2num(item{:}));
        else
            err=1;
        end
