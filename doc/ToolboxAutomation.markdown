...
exists, which points to the `bbci_tex` folder of the SVN with
'/' at the end, e.g.,
...
under `BCI_DIR/investigation/studies/''{SESSION_NAME}''` or
under `BCI_DIR/investigation/projects/''{SESSION_NAME}''`.
The LaTeX files are stored under
`TEX_DIR/studies/''{SESSION_NAME}''`, resp.
`TEX_DIR/projects/''{SESSION_NAME}''`. The (numerous) figures
that are automatically generated are stored in a subdirectory
'pics_auto' thereof, and should preferably **not** be added to the SVN.
...
**The function `proc_grandAverage`** and some other functions
...
Here, `session_name` should the variable that is returned as
second argument of the function `get_session_list` which
includes the parent directory 'studies' or 'projects', see above. The
function `list_for_tex_loop` saves a files named
`subject_loop.tex` in the corresponding session folder in
TEX_DIR. This tiny LaTeX file can be included in your LaTeX file to
...
The compilation of the LaTeX file can be run from matlab in the
following way (assuming the file above to be called
'investigate_ERPs.tex'):
