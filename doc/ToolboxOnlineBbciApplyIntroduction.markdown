...
Second case: ERP classification: The ERP response after cues are classified into
...
in modules: `source`, `signal`,
`feature`, `classifier`, `control`,
...
different fields of `bbci`. The subfield
`condition` of `control` defines under which
...
The following fields of `bbci` may be
`struct arrays`:
...
each feature may have input only from one `signal`
...
`source`, `signal`, `feature`,
`classifier`, `control`. All those fields are
`struct arrays`. Since features may be used by different
