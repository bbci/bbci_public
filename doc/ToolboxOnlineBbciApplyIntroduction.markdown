...
`bbci`. This is the input to the function
`bbci_apply.m` and it is static. (It is just modified in the
very beginning of `bbci_apply.m` by the function
`bbci_apply_setDefaults` to fill missing fields with default
values.) The variable `bbci` is typically generated by the
script `bbci_analyze` from calibration data. See also the
demos in `bbci/online_new/demos/` where some examples for
`bbci` structures are defined from the scratch. The data
structures are explained in more detail in
`bbci_apply_structures.m` and
