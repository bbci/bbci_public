...
<tr>  <td>file </td> <td> is taken relative to EEG_RAW_DIR, unless it starts with the character
...
file. Default EEG_MAT_DIR (global variable).</td> </tr>
</tbody></table>
...
<table border="1"><tbody>
<tr>  <td> .path </td> <td>  In case file does not include an absolute path, opt.path is prepended to
file. Default EEG_MAT_DIR (global variable). </a></td> </tr>
...
(see property .resolution_list). If this is possible .format is set to
...
INT16 (see property .resolution_list). For all other channels the
...
<tr>  <td> .resolution_list </td> <td> Vector of numerical values. These values are tested as resolutions to
