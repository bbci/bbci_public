%warning('use new function proc_segmentation');
...
%  MRK:  STRUCT       - class markers (see mrk_defineClasses)
%  IVAL: DOUBLE [Mx2] - time interval relative to marker [start ms, end ms].
...
%    'MarkerToSamplePosition': CHAR (default 'middle') - determines where
%                       the markers are set wrt a sample {'compatibility',
%                       'middle', 'before', 'after'}.
%    'MTSP': CHAR (default '') - short for 'MarkerToSamplePosition'.
%    'Jitter': DOUBLE [1xP] - vector of time shifts (jitter) [ms]
%    'DiscardIncompleteTrials': BOOL (default 1) - discard epochs even when the ival
%                         is not available.
%
%Returns:
%   EPO -  structure of epoched signals
%     .x     - signals (time x channels x epochs)
%     .y     - class labels (if y is a field of mrk)
%     .t     - time axis
...
%   eegfile_readBV) and converts in into data, segmented around the markers
%   as given by the mrk struct. Typically we are interested in the EEG
%   around a particular event; this function allows the large continuous
%   data structure to be reduced to the relevant data.
% 
%Examples:
%   [cnt, mrko] = eegfile_readBV(some_file);
%   chans = {'Fz', 'Pz', 'Oz'};
%   ival = [-200 800];
%   classDef = {[1:10],[11:20];'Non-target','Target'};
%   mrk = mrk_defineClasses(mrko,classDef);
%   epo = cntToEpo(cnt,mrk,ival,'chan', chans);
%
% SEE  eegfile_loadBV, eegfile_loadMatlab, mrk_defineClasses

% Benjamin Blankertz 02/03
