function xa= procutil_movingAverage(x, n, varargin)
%PROCUTIL_MOVINGAVERAGE - moving average
%
%Description:
...
%
%Synopsis:
% XMA= procutil_movingAverage(X, N, 'Property1',Value1, ...);
% XMA= procutil_movingAverage(X, N, method)
% XMA= procutil_movingAverage(X, N, method, window)
%
%Arguments:
% X:      input data matrix
% N:      length of window
...
% TolerateNans: 0 or 1. If 1, the moving average is only calculated
...
% procutil_movingAverage([x, flipud(x)], 3, 'method','centered','tolerateNans',1)
...
props= {'method', 'causal', 'CHAR(causal centered)'
        'tolerateNans', 0, 'BOOL'
        'window', 1, 'DOUBLE'
       };
opt= opt_setDefaults(opt, props, 1);
misc_checkType(x, 'DOUBLE');
misc_checkType(n, 'INT');
...
if isobject(window)
  switch(opt.method),
   ...
   case 'centered'
    window = window((1:n)'/(n+1)*pi)
  end
end

if length(window)==1 & ~opt.tolerateNans,
  x = x*window;
  switch(opt.method),
   case 'centered',
    w0= -ceil((n-1)/2);
