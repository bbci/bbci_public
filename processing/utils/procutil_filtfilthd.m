function x=procutil_filtfilthd(varargin)
% PROCUTIL_FILTFILTHD Zero-phase digital filtering with dfilt objects.
%
...
%
% Examples:
% x=PROCUTIL_FILTFILTHD(Hd, x)
% x=PROCUTIL_FILTFILTHD(Hd, x, method)
% where Hd is a dfilt object and x is the input data. If x is a matrix,
% each column will be filtered.
%
% ------------------------------------------------------------------------
% The filter states of Hd on entry to PROCUTIL_FILTFILTHD will be used at the
...
% calling PROCUTIL_FILTFILTHD [e.g. using reset(Hd);]
% ------------------------------------------------------------------------
%
% x=PROCUTIL_FILTFILTHD(b, a, x) 
% x=PROCUTIL_FILTFILTHD(b, a, x, method)
%           format is also supported.
%
% x=PROCUTIL_FILTFILTHD(...., IMPULSELENGTH)
...
if (m>1) && (n>1) 
    for i=1:n
        x(:,i)=procutil_filtfilthd(Hd, x(:,i), method, nfact);
    end
    return
end
