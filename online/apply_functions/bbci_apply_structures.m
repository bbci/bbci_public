...
%  .acquire_fcn   [FUNC, default '@bbci_acquire_bv']
...
%  .clab          [CELL ARRAY of CHAR, default {'*'}]
%  .record_signals  [BOOL, default 0] specifies whether bbci_apply should
%                 record the signals to file.
%  .record_basename  [CHAR, default ''] filename for recording the signals.
%                 A counter number is appended to avoid overwriting other files.
%  .record_param  [CELL, default {}] Cell array of parameters that are
%                 passed to the recording function, see bbci_apply_recordSignals.
%  .marker_mapping_fcn  [FUNC, default []] handle of a function that is used
%                 to map markers from CHAR to DOUBLE.
%                 Typically not required
%  .log           [STRUCT] This field specifies, whether source-specific
%                 information should be logged:
%                    'output': 0, screen, file, or screen&file: see bbci.log
%                    'data_packets': write a log message, if the length of an
%                        acquired block is larger than .min_blocklength,
%                        default 0
%                    'markers': writes a log message for each received marker
%                    'time_fmt': specifies the time format for source-log
%                        messages, default '%08.3fs'.
%
%bbci.marker - Defines how the acquired markers are stored
%  * struct with fields:
%  .queue_length  [INT, default 100] Specifies how many markers are stored
%                 in the marker queue (see data.marker). The markers in the
%                 queue are available for queries and evaluating conditions, see
%                 bbci_apply_queryMarker.
%
%bbci.signal - Defines how the continuous signals are preprocessed and
%                 stored into the ring buffer
%  * struct array with fields:
%  .source        [INT[1], default 1] specifies from which input source k
%                 (defined by bbci.source(k)) this signal gets input.
%  .proc          [CELL ARRAY, one cell per proc function,
%                  each CELL is either a FUNC, or a
%                  CELL ARRAY {FNC, PARAM, ...}, where FNC is FUNC and
%                  PARAM is a CELL ARRAY of parameters to the function;
%                  default {}]
%  .buffer_size   [INT[1], default 5000] specifies in msec the length of the
%                 (ring) buffer, in which signals are stored. This is the 
%                 buffer of signals from which features can be calculated
%  .clab          [CELL ARRAY of CHAR, default {'*'}] defines the set of
%                 channels that is to be contained in this 'signal'.
%
%bbci.feature - Defines extraction of features from continuous signals
%  * struct array with fields:
%  .signal        [INT, default 1] specifies from which signal k (defined
%                 by bbci.signal(k)) this feature is extracted.
%  .ival          DOUBLE[2] - specifying the time interval [start_msec end_msec]
%                 that is extracted from the signal to calculate this feature.
%  .proc          [CELL ARRAY, one cell per proc function,
%                  each CELL is either a FUNC, or a
%                  CELL ARRAY {FNC, PARAM, ...}, where FNC is FUNC and
%                  PARAM is a CELL ARRAY of parameters to the function;
...
%                 that was acquired
%     .marker        CELL of CHARs (??or rather [vector of DOUBLE]??)
...
%  .desc         [CELL: 1xBBCI.MARKER.QUEUELENGTH of CHARs] marker
%                descriptors OR
