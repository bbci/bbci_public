...
for k= 1:length(BS),
  ...
  DS.state.running= 1;
  DS.state.chan_sel= util_chanind(DS.state.clab, BS(k).clab);
...
if ~isempty(header_str),
  bbci= bbci_log_addHeaderLine(bbci, header_str);
end
...
for k= 1:length(BS),
  ...
  DB.chidx= util_chanind(DS, BS(k).clab);
...
%% Initialize 'feedback'
void_feedback= struct('state',struct, 'opt',[], 'event',[]);
data.feedback= repmat(void_feedback, [length(bbci.feedback) 1]);
for k= 1:length(bbci.feedback),
  data.feedback(k)= bbci_apply_sendControl('init', bbci.feedback(k), ...
                                                   data.feedback(k));
end
